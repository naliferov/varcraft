...
const httpGetFile = async (x) => {
  ...
  try {
    return {
      file: await fs({ get: { path: '.' + ctx.url.pathname } }),
      mime,
    }
  } catch (e) {
...
const getHtml = async (x) => {
  return {
    v: await fs({ get: { path: './index.html' } }),
    isHtml: true,
  }
}

const mkResp = async ({ statusCode = 200, mime, v, isBin }) => {
...
const getBody = async (ctx, limitMb = 12) => {
...
const httpHandler = async (x) => {
    ...
    const r = await httpGetFile({ ctx })
    if (r.fileNotFound) return mkResp({ v: 'File not found' })
    else if (r.file) return mkResp({ v: r.file, mime: r.mime, isBin: true })

    let msg = await getBody(ctx)
    if (!msg) msg = {}
    if (msg.err) return mkResp({ v: msg.err })
    ...
    let resp = null
    if (Object.keys(msg).length < 1) resp = await getHtml(msg.data)

    if (!resp) return mkResp({ v: { defaultResponse: true } })
    if (resp.isHtml) return mkResp({ v: resp.v, mime: 'text/html' })
    return mkResp({ v: resp })
}
...
server.on('request', async (rq, rs) => {
  ...
  try {
    const r = await httpHandler({ runtimeCtx: ctx, rq })
